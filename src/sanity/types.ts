...
// Query: *[_type == "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || author -> name match $search]| order(_createdAt desc) {  _id,    title,    slug,    _createdAt,    author ->{      _id, name, bio, image    },    views, description, category, image}
...
// Query: *[_type == "startup" && _id == $id] [0]{  _id,    title,    slug,    _createdAt,    author ->{      _id, name, bio, image, username,    },    views, description, category, image, pitch}
